...
module.exports.deactiveSubscription = async (req, res) => {
  try {
    ...
    if (customerSubscription.customerId !== customer.id)
      return res.status(403).send({ message: "you dont access to this data" });
    if (!customerSubscription.isActive)
      return res
        .status(400)
        .send({ message: "customer subscription is already deactivated" });

    await CustomerSubscriptions.updateOne({ isActive: false }, id);

    res
      .status(200)
      .send({ message: "customer subscription successfully deactivated" });
  } catch (error) {
